...
function setup() {
  ...
  invisibleGround = createSprite(windowWidth/2,windowHeight/2+5,windowWidth,10);
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if((keyDown("space")|| touches.lenght > 0) && trex.y >= windowHeight/2-30) {
      trex.velocityY = -12;
      touches = [];
    }
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    ...
    cloud.y = Math.round(random(0,windowHeight/2-100));
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(windowWidth,windowHeight/2-18,10,40);
    //obstacle.debug = true;
